          ...
          <View style={styles.profileSection}>
            ...
            {(user.firstName || user.lastName) && (
              <View style={styles.profileRow}>
                ...
                <Text style={styles.profileValue}>{[user.firstName, user.lastName].filter(Boolean).join(" ")}</Text>
              </View>
            )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.distributionSection}>
            {Object.entries(stats.taskStatusDistribution).map(([status, count]) => {
              const percentage = stats.totalTasks > 0 ? ((count / stats.totalTasks) * 100).toFixed(1) : "0.0";
              return (
        ...
        <View style={styles.section}>
          ...
          <View style={styles.summaryBox}>
            <Text style={styles.summaryText}>
              ...
              {"\n\n"}
              This report shows {userName ? `${userName}'s` : "the member's"} performance metrics including task
              completion rates, engagement scores, and activity distribution.
              {"\n\n"}
              Completed This Month: {stats.completedThisMonth} task{stats.completedThisMonth !== 1 ? "s" : ""}
              {"\n"}
              Tasks Pending:{" "}
              {(stats.taskStatusDistribution.TODO || 0) + (stats.taskStatusDistribution.IN_PROGRESS || 0)}
              {"\n"}
